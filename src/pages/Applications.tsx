...
import { LayoutGrid, List, Map, Search, Filter, MapPin, Building2, DollarSign } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 items-center justify-between">
        ...
        <div className="flex gap-3 flex-1 max-w-md">
          ...
          <ButtonRetro variant="outline" size="icon"><Filter className="h-4 w-4" /></ButtonRetro>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'Saved', 'Applied', 'Interview', 'Offer', 'Rejected', 'Ghosted'].map(s => (
          <button key={s} onClick={() => setStatusFilter(s as any)} className={cn(
      ...
      {viewMode === 'list' && (
        <CardRetro className="overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {filteredApps.map(app => (
                <tr key={app.id} className="border-t-2 border-border hover:bg-muted/50 cursor-pointer" onClick={() => window.location.href = `/applications/${app.id}`}>
      ...
      {viewMode === 'map' && (
        <CardRetro className="p-12 text-center border-dashed">
          ...
          <div className="flex gap-2 justify-center mt-4">
            <span className="px-3 py-1 bg-muted rounded-full text-sm border-2 border-border">📍 Remote</span>
            <span className="px-3 py-1 bg-muted rounded-full text-sm border-2 border-border">📍 San Francisco</span>
            <span className="px-3 py-1 bg-muted rounded-full text-sm border-2 border-border">📍 New York</span>
          </div>
